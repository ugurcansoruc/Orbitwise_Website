...
function NewsLatterBox({
  ...
  const [_emailSave, setEmailSave] = useState<boolean | null>(null);
  const [formData, setFormData] = useState<MailDocument | null>(null);

  const { register, handleSubmit } = useForm<MailDocument>();
    ...
    <>
      ...
      >
        ...
        <p className="mb-11 border-b border-body-color border-opacity-25 pb-11 text-base font-medium leading-relaxed text-body-color dark:border-white dark:border-opacity-25">
          {_NewsLatterBoxT.Content}
        </p>
        <form onSubmit={handleSubmit(onSubmit)}>
          <input
            type="text"
            placeholder={_NewsLatterBoxT.PlaceholderName}
          ...
          <input
            type="email"
            placeholder={_NewsLatterBoxT.PlaceholderMail}
            className="mb-4 w-full rounded-md border border-body-color border-opacity-10 px-6 py-3 text-base font-medium text-body-color placeholder-body-color outline-none focus:border-primary focus:border-opacity-100 focus-visible:shadow-none dark:border-white dark:border-opacity-10 dark:bg-[#242B51] focus:dark:border-opacity-50"
            {...register("email", { required: true })}
          />
          <input
            type="submit"
            value={_NewsLatterBoxT.Subscribe}
            className="duration-80 mb-4 w-full cursor-pointer rounded-md border border-transparent bg-primary px-6 py-3 text-center text-base font-medium text-white outline-none transition ease-in-out hover:bg-opacity-80 hover:shadow-signUp focus-visible:shadow-none"
          />
          <p className="text-center text-base font-medium leading-relaxed text-body-color">
            {_NewsLatterBoxT.SubContent}
          </p>
        </form>
